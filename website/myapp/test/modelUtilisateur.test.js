...
describe("Model Tests", () => {
    ...
    test("check role user", async () => {
        const newUser = [
            "testCheckType@test.com",
            "TestInfos",
            "Jean-Test",
            "06-13-82-58-33",
            "c'estletestchecktype"
        ];
        await model.create(newUser[0], newUser[1], newUser[2], newUser[3], newUser[4]);
        const user = await model.read(newUser[0]);
        const result = await model.checkRole(user.id_utilisateur)
        await model.fullDelete(newUser[0]);
